...
print("Similarity Comparison begin")
#*****************************************
...
# randomly predict value for Null
print(doc_test_pred[!(doc_test_pred)%in% unlist(doc_test_pred)])
pred <- as.character(doc_test_pred)
nullidx <- which(pred=="NULL")
pred[nullidx] <- sample(unique(testLabel$path),replace = FALSE)
print(pred[nullidx])
pred <- data.frame(path=pred,id=names(doc_test_pred))
sorted.pred<-as.character(pred[order(pred$id,decreasing = FALSE),c("path")])

if(enableO4==TRUE){
  DT1 <- data.table(prediction=sorted.pred,truth=as.character(testLabel$path))
...
truth <- as.factor(testLabel$path)
...
TN <- c()
for (k in seq(1,nrow(tax))){
  TN <-c(TN,sum(colSums(tax[-k,-k])))
}
